...
int envioSms(void)
{
    // envio do sms com as info de geolocalizacao
    // Get data from gps
    char buffer[1000];
    int i = 0;
    int j = 0;
...
int main()
{
    ...
    for (;;)
    {
        ...
        if ((digitalRead(sensorPIR) == 0) && (processamento == 0))
        {
            sleep(1);
            printf("Sem presença...\n");
            continue;
        }
        else if ((digitalRead(sensorPIR) == 1 && processamento == 0))
        {
        ...
        for (size_t i = 0; i < faces.size(); i++)
        {
            ...
            if (labels.find(predicted) == labels.end() || confidence > 18)
        ...
        // envio
        if (unknowTrue >= 30)
        { // Verifica se teve 30 frames com invasor e envia o sms
            digitalWrite(ledAct, HIGH);
            envioSms();
